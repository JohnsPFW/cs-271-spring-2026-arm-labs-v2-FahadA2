...
Binary (32-bit):
```
f_    2_    a_    0_    1_    f_    e_    5_
1111 0010  1010  0000  0001  1111  1110  0101
```
...
|    |    |    |       |       |       |       |     |     |
  1    1    1    10010    10    00000   000111  11111 00101
- `Rm` (binary) = 00000
- `Rn` (binary) = 11111
- `Rd` (binary) = 00101
...
Binary (32-bit):
```
9_    2_    1_    e_    2_    c_    a_    6_
1001 0010  0001  1110  0010  1100  1010  0110
```
...
| sf | op | S  | 01011 | shift | Rm    | imm6  | Rn  | Rd  |
|    |    |    |       |       |       |       |     |     |
  1    0    0    10010    00     11110  001011  00101 00110
Compare the `op` and `S` bits to `ADD` above:
- How does the encoding differ to signal that condition flags should be updated?

op and s are both 0 now compared to being both 1 in ADD. This is done to identify the ADD operation ad addition. This is done to signal that condition flags should be updated. 
...
movz = #0xffff (000000000000000000000000000ffff)
movk = #0xff, lsl #16 = 
